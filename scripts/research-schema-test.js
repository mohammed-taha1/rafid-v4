...
const scores=scoreAnalysis(elements); assert.equal(scores.technical.score,15); assert.equal(scores.funding.score,10); assert.equal(scores.funding.dimensions.find((item)=>item.id==="fundingFit").score,0); const analysis=createAnalysis({elements}); assert.equal(validateAnalysis(analysis).valid,true); analysis.technicalReadiness.dimensions[0].explanation=""; assert.equal(validateAnalysis(analysis).valid,false); console.log("Rafid research schema and scoring tests passed.");
