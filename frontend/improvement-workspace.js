  ...
  async function reassess(state) {
    ...
    try {
      ...
      for (;;) {
        ...
        if (payload.job.status === "completed") { recordRound(payload.job.result.assessment); sessionStorage.removeItem(stateKey); window.RafidApp?.showMatchResult({ opportunity: payload.job.result.opportunity, assessment: payload.job.result.assessment, meta: { job: payload.job.timings_ms, chunks: payload.job.chunk_metrics } }); return; }
